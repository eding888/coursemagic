...
sessionManagementRouter.get('/getSession', async (request: Request, response: Response) => {
  const token = tokens.create(Bun.env.SECRET || "bomboclaattttt");
  response.status(200).json({ csrf: token });
});

// Logs the user out
sessionManagementRouter.get('/logout', async (request: Request, response: Response) => {
